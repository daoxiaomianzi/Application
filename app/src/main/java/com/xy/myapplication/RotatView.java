...
//                        }
                        if (deta_degree < 0) {
                            deta_degree += 20;
                            Log.d("反向回正角度后", deta_degree + "");
                            if (deta_degree > 0) {
                                deta_degree = 0;
                            } else {
                                handler.sendEmptyMessageDelayed(play, delayedTime);
                            }
                            Log.d("反向回正角度", deta_degree + "");

                        } else {
                            deta_degree -= 20;
                            Log.d("正向回正角度后", deta_degree + "");
                            if (deta_degree < 0) {
                                deta_degree = 0;
                            } else {
                                handler.sendEmptyMessageDelayed(play, delayedTime);
                            }
                            Log.d("正向回正角度", deta_degree + "");
                        }
//                        addDegree((float) (speed * detaTime + (a * detaTime * detaTime) / 2));
    ...
    private void addDegree(float added) {
        deta_degree += added;
//        if (deta_degree > 540 || deta_degree < -540) {
//            deta_degree = deta_degree % 360;
//        }
        Log.d("ssss", deta_degree + "");
